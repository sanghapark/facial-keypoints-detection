...
class FacialKeypointsCnnModel:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name):
            ...
            # Filter Size = [2, 2, 1, 100] 
            # 100개의 2x2x1 filters
            W1 = tf.Variable(tf.random_normal([2, 2, 1, 100], stddev=0.01))
            ...
            # L2 ImgIn shape=(?, 96, 96, 100)
            W2 = tf.Variable(tf.random_normal([2, 2, 100, 200], stddev=0.01))
            ...
            W3 = tf.Variable(tf.random_normal([2, 2, 200, 400], stddev=0.01))
...
for batch_index in range(1, int(np.ceil(X_test.shape[0]/batch_size)+1)):
    ...
    Y_predicted = m1.predict(X_test[start:end,], keep_prop=1.0)
